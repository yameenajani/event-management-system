...
if (isset($_REQUEST["btnUpdate"])) {
    if ($_REQUEST["btnUpdate"] == "Update") {
        $sql = "UPDATE `EM_Branches` SET `Name`='" . $_POST['txtBranchName'] . "' WHERE 1 and ID = " . $_REQUEST["id"];
    ...
    if (isset($_REQUEST["btnSubmit"])) {
        $sql = "INSERT INTO `EM_Branches`(`Name`) VALUES (";
        $sql .= "'" . $_POST['txtBranchName'] . "')";
        $rsl = $db_conn->query($sql);
...
if (isset($_REQUEST["id"])) {
    $data = "SELECT * FROM `EM_Branches` WHERE 1 AND isDeleted = 'N' AND ID =" . $_REQUEST["id"];
    ...
    $category = $_REQUEST['txtBranchName'];
}
?>
...
<body>
    <form class="dataForm" method="POST" name="myForm">
        ...
        <div class="form-group row">
            <label for="Branch Name" class="col-sm-2 offset-1 col-form-label">Branch Name</label>
            <div class="col-sm-6">
                <input type="text" class="form-control" name="txtBranchName" placeholder="Branch Name" value="<?php print $branch; ?>" required>
            </div>
        </div>
        <br>
        <div class="form-group row">
