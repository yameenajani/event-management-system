...
if (isset($_REQUEST["btnUpdate"])) {
    if ($_REQUEST["btnUpdate"] == "Update") {
        $sql = "UPDATE `EM_Classes` SET `Year`='" . $_POST['txtClassName'] . "' WHERE 1 and ID = " . $_REQUEST["id"];
    ...
    if (isset($_REQUEST["btnSubmit"])) {
        $sql = "INSERT INTO `EM_Classes`(`Year`) VALUES (";
        $sql .= "'" . $_POST['txtClassName'] . "')";
        $rsl = $db_conn->query($sql);
...
if (isset($_REQUEST["id"])) {
    $data = "SELECT * FROM `EM_Classes` WHERE 1 AND isDeleted = 'N' AND ID =" . $_REQUEST["id"];
    ...
    $category = $_REQUEST['txtClassName'];
}
?>
...
<body>
    <form class="dataForm" method="POST" name="myForm">
        ...
        <div class="form-group row">
            <label for="Branch Name" class="col-sm-2 offset-1 col-form-label">Class Name</label>
            <div class="col-sm-6">
                <input type="text" class="form-control" name="txtClassName" placeholder="Class Name" value="<?php print $class; ?>" required>
            </div>
        </div>
        <br>
        <div class="form-group row">
